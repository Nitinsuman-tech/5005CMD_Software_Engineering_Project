...
export default function ChairmanLibrary() {
    ...
    try {
      ...
      const contribItems = contribSnap.docs.map((d) => {
        ...
        return {
          ...
          studentEmail: data.studentEmail || data.contributorName || "Anonymous",
          ...
          isVisibleInCommunity: data.isVisibleInCommunity || false,
      ...
      console.error("Error fetching chairman library:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveFeedback = async () => {
    ...
    try {
      const chairmanName = userData?.displayName || user.email;
      const newFeedback = {
        teacherId: user.uid,
        teacherEmail: chairmanName,
      ...
      if (selectedItem.studentId) {
        createNotification(
          ...
          `Chairman ${chairmanName} has reviewed your submission: "${selectedItem.title}"`,
          null, 
          { type: "feedback", senderId: user.uid, orgId: orgId, contributionId: selectedItem.id }
        );
      }
  ...
  /**
   * Publish to Community Library — sets isVisibleInCommunity: true.
   */
  const handlePublishToCommunity = async () => {
    ...
    try {
      ...
      if (selectedItem.studentId) {
        createNotification(
          selectedItem.studentId,
          `Your submission "${selectedItem.title}" has been published to the Community Library by ${chairmanName}!`,
          null,
          { type: "published", senderId: user.uid, orgId: orgId, contributionId: selectedItem.id }
        );
      }
      ...
      setItems((prev) =>
        prev.map((i) =>
          i.id === selectedItem.id ? { ...i, isVisibleInCommunity: true, status: newStatus } : i
        )
      );
      setSelectedItem((prev) => ({ ...prev, isVisibleInCommunity: true, status: newStatus }));
    } catch (err) {
      console.error("Error publishing to community library:", err);
    } finally {
      setPublishing(false);
    }
  };
    ...
    try {
      ...
      if (selectedItem.studentId) {
        createNotification(
          selectedItem.studentId,
          `Your submission "${selectedItem.title}" has been published to the Public Showcase by ${chairmanName}!`,
          null,
          { type: "published", senderId: user.uid, orgId: orgId, contributionId: selectedItem.id }
        );
      }
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((i) =>
          i.id === selectedItem.id
            ? { ...i, isVisibleInCommunity: false, isVisibleInPublic: false, isPublished: false, isPublic: false }
            : i
        )
      );
      setSelectedItem((prev) => ({
  ...
  const publishedCommunityCount = items.filter((i) => i.isVisibleInCommunity).length;
  ...
  return (
    <div className="school-library-page">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div>
            ...
            <p className="school-library-desc">
              Review member submissions. Provide feedback and control what appears in the Community Library and Public Showcase.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="review-stats-bar">
        ...
        <span className="review-stat">🏫 Community Library: {publishedCommunityCount}</span>
            ...
            {!selectedItem ? (
              ...
              <>
                ...
                <div className="sl-publish-section" style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginTop: "16px", padding: "16px", background: "#f8faf8", borderRadius: "10px", border: "1px solid #e0e0e0" }}>
                  ...
                  {selectedItem.isVisibleInCommunity ? (
                    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <span style={{ fontSize: "13px", color: "#2E7D32", fontWeight: 600 }}>✅ In Community Library</span>
                    </div>
                  ) : (
                    <button
                      onClick={handlePublishToCommunity}
                      disabled={publishing}
                      title="Make visible in Community Library for members"
                      style={{
                        padding: "8px 16px",
                        background: "#e8f5e9",
                        color: "#2E7D32",
                    ...
                    >
                      {publishing ? "Publishing..." : "🏫 Publish to Community Library"}
                    </button>
                  )}
                  ...
                  {(selectedItem.isVisibleInCommunity || selectedItem.isVisibleInPublic) && (
                    <button
